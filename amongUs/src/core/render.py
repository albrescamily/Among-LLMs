...
def draw_world(game):
    ...
    for sprite in game.all_sprites:
        ...
        # the hit box is per sprite, so this one belongs in the loop
        if game.draw_debug:
...
def draw_debug_overlay(game):
    """Wall outlines and the tile grid, once for the whole frame.

    These three used to sit inside the sprite loop, so they were redrawn once
    per sprite -- the same pixels, twenty-five times over.
    """
    for wall in game.walls:
        pg.draw.rect(game.screen, YELLOW, game.camera.apply_rect(wall.rect), 1)
