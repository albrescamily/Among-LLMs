...
class Game:
    ...
    def _apply_remote_row(self, p, player_id):
        """One player's row out of the server's broadcast, applied locally.

        Everything is addressed by position, so the bare integer indexes are
        the wire format; protocol.py names the chat and colour ones.

        The sprite the peer is showing arrives as a *string of Python source*
        that gets eval()'d below, and that source says `self.Players[p[0]]...`,
        so it reads the names `self` and `p` straight out of this frame.
        """
        ...
        while self.playing:
            ...
            for inm in ins:
                ...
                if gameEvent[0] == 'player locations':
                    ...
                    # iterating gameEvent
                    for p in gameEvent:
                        self._apply_remote_row(p, player_id)
